...
const getReformulationByScore = (scores: any): string => {
  ...
  // Ensuite insultes sévères
  if (severeToxicity && severeToxicity > 0.6) {
    const reformulations = [
...
export const moderateTextWithAPI = async (text: string): Promise<ModerationResult> => {
  ...
  try {
    ...
    // Seuil de toxicité: 0.7 = bloqué
    const isToxic = scores.toxicity > 0.7 || 
                    (scores.severeToxicity && scores.severeToxicity > 0.6) ||
                    (scores.threat && scores.threat > 0.6);
